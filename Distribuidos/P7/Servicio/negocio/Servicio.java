...
public class Servicio{
  	...
  	public Response alta_usuario(@FormParam("usuario") Usuario usuario) throws Exception{
		...
		try{
			...
			try{
				...
				try{if (rs.next())		return Response.status(400).entity(j.toJson(new Error("El email ya existe"))).build();}
				finally{rs.close();}
	...
	public Response borra_usuario(@FormParam("email") String email) throws Exception{
		...
		try{PreparedStatement stmt_1 = conexion.prepareStatement("DELETE FROM fotos_usuarios WHERE id_usuario=(SELECT id_usuario FROM usuarios WHERE email=?)");
			try{
				stmt_1.setString(1,email);
				stmt_1.executeUpdate();
			}finally{stmt_1.close();}
	...
	public Response borrar_usuarios() throws Exception{
		Connection conexion= pool.getConnection();
		try{
			PreparedStatement stmt = conexion.prepareStatement("DELETE FROM usuarios");
